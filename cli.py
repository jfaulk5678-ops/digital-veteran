...
def main():
    ...
    parser.add_argument(
        "--generate-leads", type=int, help="Generate N leads based on ICP"
    )
    parser.add_argument(
        "--add-feedback", type=str, help="Add manual feedback (use single quotes)"
    )
    parser.add_argument("--reflect", action="store_true", help="Run reflection cycle")
    parser.add_argument(
        "--import-crm", action="store_true", help="Import from CRM and learn"
    )
    parser.add_argument(
        "--show-icp", action="store_true", help="Show ICP recommendations"
    )
    ...
    try:
        ...
        if args.demo:
            ...
            for lead in scored_leads:
                print(
                    f"   {lead['company_name']} - Confidence: {lead['confidence_score']:.1%}"
                )
