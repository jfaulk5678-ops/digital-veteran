...
class LeadSourcingAgent:
    ...
    def score_leads(self, leads: list):
        ...
        for lead in leads:
            lead['recommendation'] = "HIGH" if lead['confidence_score'] > 0.7 else "MEDIUM" if lead['confidence_score'] > 0.5 else "LOW"
...
def test_lead_generation():
    ...
    for lead in scored_leads:
        print(f"   {lead['company_name']} - Size: {lead['company_size']} - Confidence: {lead['confidence_score']:.1%} - {lead['recommendation']}")
